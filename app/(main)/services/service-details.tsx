...
const ServiceDetails = () => {
  ...
  const { id, name, isSleepingPod, serviceName, isPartner, category_id, category_name } = params;
  const isTravloungePartner = isPartner === 'true';
  const isCarwash = category_name === 'Car Wash';
  const isCafe = category_name === 'Cafe';
  const isBuffet = category_name === 'Buffet';
      ...
      {data && (
        <ScrollView>
          <ServiceDetailCard
            ...
            offerPercentage={data?.user_percentage}
          />
          ...
          {(isCafe || isBuffet) && data?.is_partner && <CafeCard service={category_name} />}
